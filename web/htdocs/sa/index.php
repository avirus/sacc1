...
echo "<br>";
	...
	echo "importing ";			
			 ...
			 $email= $arr['email'];
			 $timeacl= $arr['timeacl'];
			 $vname= addslashes($arr['vname']);
			 $vname=iconv("UTF-8","KOI8-R", $vname);
			 echo $login;
	$acl = option('def_timeacl',$link);	 
	$req="INSERT INTO users (login, used, quota, dquota, email, descr, aid) VALUES('$login', $used, $limit, $limit, '" . addslashes ( $email ) . "', '" . addslashes ( $vname ) . "', $acl)";
	$result = mysql_query ( $req,$link);
	if (0<mysql_errno()) {
	echo $req.mysql_error ( $link );
	};
	$req="select id from acl where sysname='$timeacl'";
	$result = mysql_query ( $req,$link);
	if (0<mysql_errno()) {
	echo $req.mysql_error ( $link );
	};
	echo " finding group $timeacl found ".mysql_num_rows($result);
	if (1==mysql_num_rows($result))
	{ // acl found
	$aclid=mysql_result ( $result, 0, "id" );		
	echo " acl=$aclid, changed rows ";
	$req="update users set aid=$aclid where login='$login'";
	$result = mysql_query ( $req,$link);
	if (0<mysql_errno()) {	echo $req.mysql_error ( $link );};
	echo mysql_affected_rows($link);
	if (1==mysql_affected_rows($link)) 
	{
		echo " setted group $timeacl<br>";
	}
	else 
	{
		echo " default group $timeacl<br>";
	};
	} else { // acl not found
		echo " exported group not found, setting default ($acl)<br>";
	};
	};
