...
// $Id: index.php 113 2012-11-20 11:42:29Z slavik $
global $settings, $link;
if (!function_exists("mysql_connect")) { die ("no mysql in php detected! please fix this, otherwise sacc system is unusable");};

require_once ("../inc/version.php");
require_once ("../inc/mysql.php");
require_once ("../inc/timer_set.php");
require_once ("../inc/functions.php");
require_once ("../inc/advfunctions.php");
...
if (isset ( $PHP_AUTH_USER )) {
	$ok = false;
	if (auth_adm ( $PHP_AUTH_USER, $PHP_AUTH_PW, $link )) {
		$ok = true;
	}
	if (! $ok) {
		if (isset ( $try )) {
...
# --------------------- MAIN --------------------
// FIXME: reopen connection, for compat
//mysql_close($link);
//$link = db_connect ();
...
if ($type == "create") {
	...
	echo "$msg
          ...
          <TR>
            ...
            <TD><INPUT TYPE=text CLASS=\"input\" NAME=userid VALUE=\"" . htmlspecialchars($userid) . "\"></TD>
            <TD><SPAN CLASS=\"tahoma11px\">&nbsp;&nbsp;&nbsp;&nbsp;$ad_descr_like</SPAN></TD>
          </TR>
          <TR>
            <TD>
            <INPUT TYPE=submit CLASS=\"inputsubmit\" ID=\"bg_green\" NAME=searchbutton VALUE=\"$ad_search\">&nbsp;&nbsp;&nbsp;&nbsp;
            <INPUT TYPE=hidden NAME=type VALUE=\"create\">
            </TD>
          </TR>
          </FORM>\n
          </TABLE>\n";
          # -- Нажата кнопка "Поиск в Active Directory" ("adsearch") --
          if ( isset($userid) ) {
            print"<HR NOSHADE COLOR=#000000 SIZE=1>";
            //print"$userid";
            show_adsearch($userid);
          }
...
<td><select name=acl style=\"font-weight: bold\">";
   ...
   <br>";
	//echo $todo;
	echo "alter <select name=todo style=\"font-weight: bold\">";
	echo "<option value=\"1\"";
	echo ($todo == 1) ? "selected" : " ";
	echo ">create and back to main";
	echo "<option value=\"0\"";
	echo ($todo == 0) ? "selected" : " ";
	echo ">create and edit";
	echo "<option value=\"3\"";
	echo ($todo == 3) ? "selected" : " ";
	echo ">show this form again";
	echo "</select>
          <BR>
...
elseif ($type == "edit") {
	...
	global $view_user, $view_login, $view_traf, $view_quota, $std_limit, $passwd_like_1,$passwd_like_2;
	echo "<FORM ACTION=\"index.php\" METHOD=get>
          <INPUT TYPE=hidden NAME=type VALUE=\"edit\">
...
<option value=\"2\" ";
	...
	echo ">$web_status_negative, $web_enable $web_at_eom
<option value=\"3\" ";
	if ($status == "3") {
		echo "selected";
	}
	echo ">$web_status_negative, $edit_delete $web_at_eom
</select></td>";
	echo "</tr>
<tr>
          </TR>
          ...
          <INPUT TYPE=button CLASS=\"inputsubmit\" ID=\"bg_red\"   NAME=deletebutton VALUE=\"delete now\" OnClick=\"if ( confirm('Warning! User &laquo;" . htmlspecialchars ( $other ) . "&raquo; will be removed without an opportunity of restoration!') ) window.location='index.php?type=edit&killbutton=Удалить&id=$id'; \">";
	if ($auth_mode == 1) {
...
elseif ( $type == "sites" )
{
    ...
    $site    = stripslashes(trim($_GET['site']));
    show_head();
    if($site){
      echo "<A HREF=\"index.php#$id\">&lt;&lt;</A>\n&nbsp;\n";
      echo "<A HREF=\"index.php?type=alldetail&site=$site\">&gt;&gt;</A>\n<BR>\n";
    } else {
      echo "<A HREF=\"index.php#$id\">&lt;&lt;</A>\n<BR>\n";
    }
    show_info($link, $id);
    ...
    show_service_stat($link, $id);
    echo "</td></tr></table>";
    if (!isset($no_mail))  {show_mailstat($link, $id);};
    show_tail();
    mysql_close($link);
    exit;
}

# ------------ Просмотр адресов посещенных сайтов (событие "allsites") ------------
elseif ( $type == "allsites" )
{
    $id    = stripslashes(trim($_GET['id']));
    show_head();
    ...
    echo "<HR NOSHADE COLOR=#000000 SIZE=1>\n<BR>\n";
    echo "<table><tr><td valign=top>";
    //show_all_stat($link, $id);
    show_all_stat($link, $id);
    echo "</td><td valign=top>";
    show_all_service_stat($link, $id);
    echo "</td></tr></table>";
    if (!isset($no_mail))  {show_mailstat($link, $id);};
    show_tail();
    mysql_close($link);
...
# ------------ Просмотр адресов посещенных пользователем сайтов за прошедшие месяцы (событие "shist") ------------
elseif ( $type == "shist" )
{
    $id    = stripslashes(trim($_GET['id']));
    $site    = stripslashes(trim($_GET['site']));
    ...
    show_history_info($link, $id);
    echo "<HR NOSHADE COLOR=#000000 SIZE=1>\n<BR>\n";
    show_history_stat($link, $id);
    if (!isset($no_mail))  {show_mailstat($link, $id);};
    show_tail();
    mysql_close($link);
    exit;
...
# ------------ Просмотр адресов посещенных сайтов за прошедшие месяцы (событие "allshist") ------------
elseif ( $type == "allshist" )
{
    $id    = stripslashes(trim($_GET['id']));
    show_head();
    //echo "<A HREF=\"index.php\">&lt;&lt;</A>\n<BR>\n";
    echo "<A HREF=\"?type=uhist&utime=$utime\">&lt;&lt;</A>\n<BR>\n";
    //$id=244;
    //print"link=$link, id=$id";
    //show_info($link, $id);
    echo "<HR NOSHADE COLOR=#000000 SIZE=1>\n<BR>\n";
...
# ------------ Просмотр служебных запросов пользователя (событие "servicedetail") ------------
...
# ------------ Просмотр служебных запросов пользователя (событие "servicealldetail") ------------
...

{
    ...
    //show_info($link, $id);
    echo "<HR NOSHADE COLOR=#000000 SIZE=1>\n<BR>\n";
    //show_all_service_detail($link, $_GET['id'], $_GET['site']);
    show_all_service_detail($link, $_GET['site']);
    show_tail();
    mysql_close($link);
    exit;
}
# ------------ Просмотр всех запросов  (событие "alldetail") ------------
elseif (( $type == "alldetail" ) and ($detailed==1))

{
    show_head();
    echo "<A HREF=\"index.php?type=allsites\">&lt;&lt;</A>\n<BR>\n";
    #show_info($link, $id);
    echo "<HR NOSHADE COLOR=#000000 SIZE=1>\n<BR>\n";
    show_all_detail($link, $_GET['id'], $_GET['site']);
    show_tail();
    mysql_close($link);
    exit;
}
...
elseif ( $type == "snmp"){
  show_head();
  echo "<H1>SNMP cisco stat</H1>\n";
  show_help();

  echo "<center><TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD BGCOLOR=#93BEE2>";
  echo "<TABLE BORDER=0 CELLPADDING=1 CELLSPACING=1>";
  echo "<TR VALIGN=TOP><TD HEIGHT=18 ALIGN=CENTER COLSPAN=4>Current Month - 2 </td></tr>";
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_eth.log.1");
  echo "</tr><TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_ser.log.1");
  echo "</tr>";
  echo "</table></TD></TR></TABLE><br>";

  echo "<center><TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD BGCOLOR=#93BEE2>";
  echo "<TABLE BORDER=0 CELLPADDING=1 CELLSPACING=1>";
  echo "<TR VALIGN=TOP><TD HEIGHT=18 ALIGN=CENTER COLSPAN=4>Previous Month </td></tr>";
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_eth.log.0");
  echo "</tr><TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_ser.log.0");
  echo "</tr>";
  echo "</table></TD></TR></TABLE><br>";

  echo "<center><TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD BGCOLOR=#93BEE2>";
  echo "<TABLE BORDER=0 CELLPADDING=1 CELLSPACING=1>";
  echo "<TR VALIGN=TOP><TD HEIGHT=18 ALIGN=CENTER COLSPAN=4>Current Month </td></tr>";
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_eth.log");
  echo "</tr><TR BGCOLOR=#FFF7E5>";
  passthru("/root/SnmpGetTraf/sum.pl /root/SnmpGetTraf/sum.cfg snmpgettraf_ser.log");
  echo "</tr>";
  echo "</table></TD></TR></TABLE><br>";
...
elseif ( $type == "netstat"){
  show_head();
  echo "<H1>Netstat interface Info</H1>\n";
  show_help();
  ...
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/NetstatGetTraf/sum.pl /root/NetstatGetTraf/sum.cfg netstatgettraf_em0.log.1");
  ...
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/NetstatGetTraf/sum.pl /root/NetstatGetTraf/sum.cfg netstatgettraf_em0.log.0");
  echo "</tr></table></TD></TR></TABLE><br>";
  ...
  echo "<TR VALIGN=TOP ALIGN=CENTER><TD HEIGHT=18>Interface</td><td>In</td><td>Out</td><td>Comment</td><td>Last Update</td></tr>";
  echo "<TR BGCOLOR=#FFF7E5>";
  passthru("/root/NetstatGetTraf/sum.pl /root/NetstatGetTraf/sum.cfg netstatgettraf_em0.log");
  echo "</tr></table></TD></TR></TABLE><br>";
...
//	$days = @mysql_result ( $result, 0, "_time" );
	...
	for($i = 0; $i < $days; $i ++) {
		...
		if ($totalmb>0) echo "<TR BGCOLOR=#FFF7E5 ALIGN=RIGHT>
...
elseif ($type == "hist") {
	...
	echo "<TR BGCOLOR=#FFF7E5><TD>$word_total </TD><TD>".mysql_numrows ( $result )."</TD></TR>";	
	echo "</TABLE></TD></TR></TABLE><br>";
	show_tail ();
	mysql_close ( $link );
	exit ();
...
elseif ($type == "event") {
	...
	echo "<TR BGCOLOR=#FFF7E5><TD>$web_admin_total</TD><TD>".mysql_numrows ( $result )."</TD></TR>";
	echo "</TABLE></TD></TR></TABLE><br>";
	show_tail ();
	mysql_close ( $link );
	exit ();
...
elseif ($type == "acled") {
	...
	if (isset ( $savebutton )) {
		...
		$wlst = stripslashes ( trim ( $wlst ) );
		$blst = stripslashes ( trim ( $blst ) );
		$slst = stripslashes ( trim ( $slst ) );
		if (0 != $id) {
			$result = mysql_query ( "update acl set vname='$vname', data='$data', blacklist='$blst', whitelist='$wlst', stoplist='$slst', sysname='$sysname' where id=$id", $link );
			if ((0 == mysql_affected_rows ( $link )) and (mysql_error ( $link )!='')) {
				echo "ERROR: " . mysql_error ( $link ) . " please contact your support";
				exit (); // write error and quit
			}
			;
		} else {
			$result = mysql_query ( "insert into acl (vname , data, sysname, blacklist, whitelist, stoplist) values ('$vname', '$data', '$sysname', '$blst', '$wlst', '$slst' );", $link );
	...
	if ($id > 0) {
		...
		$slst = mysql_result ( $result, 0, "stoplist" );
		$blst = mysql_result ( $result, 0, "blacklist" );
		$wlst = mysql_result ( $result, 0, "whitelist" );
	};
	echo "<TR>
	 		<INPUT TYPE=hidden NAME=id VALUE=\"$id\">
			<TD>system name<INPUT TYPE=text CLASS=\"input\" NAME=sysname VALUE=\"$sysname\"></TD></tr><tr>
            <TD>system name<INPUT TYPE=text CLASS=\"input\" NAME=sysname VALUE=\"$sysname\"></TD></tr><tr>
            <TD>friendly name<INPUT TYPE=text CLASS=\"input\" NAME=vname VALUE=\"$vname\"></TD></tr><tr>
            <TD>time acl<br>
            <textarea wrap=soft name=\"data\" rows=10 cols=60 wrap=\"physical\" tabindex=6>$data</textarea>
            <TD>stoplist (always deny)<br>
            <textarea wrap=soft name=\"slst\" rows=10 cols=60 wrap=\"physical\" tabindex=7>$slst</textarea>
            </td>
          </tr><tr>
            <TD>whitelist (always allow)<br>
            <textarea wrap=soft name=\"wlst\" rows=10 cols=60 wrap=\"physical\" tabindex=7>$wlst</textarea>
            </td>
            <TD>blacklist (deny by group_blacktime)<br>
            <textarea wrap=soft name=\"blst\" rows=10 cols=60 wrap=\"physical\" tabindex=7>$blst</textarea>
            </td>
          </TR>";
	echo "</TABLE>
	<INPUT TYPE=submit CLASS=\"inputsubmit\" ID=\"bg_green\" NAME=savebutton VALUE=\"$edit_save\">&nbsp;&nbsp;&nbsp;&nbsp;
	<INPUT TYPE=button CLASS=\"inputsubmit\" ID=\"bg_blue\"  VALUE=\"return\" onClick=\"window.location='index.php?type=acl'\">&nbsp;&nbsp;&nbsp;&nbsp;
	<INPUT TYPE=button CLASS=\"inputsubmit\" ID=\"bg_blue\"  VALUE=\"delete\" onClick=\"window.location='index.php?type=aclrm&id=$id'\">&nbsp;&nbsp;&nbsp;&nbsp;";	
	show_tail ();
	exit ( 0 );
}
// удаление ацл
elseif ($type == "aclrm") {
		$dacl=$settings ["def_timeacl"];
		$id = ( int ) stripslashes ( trim ( $id ) );
		if (0 != $id) {
			$result = mysql_query ( "delete from acl where id=$id", $link );
			if (0 == mysql_affected_rows ( $link )) {
				echo "ERROR: " . mysql_error ( $link ) . " please contact your support";
				exit (); // write error and quit
			};
		mysql_query ( "update users set aid=$dacl where aid=$id", $link );
		mysql_query ( "insert into queue (command, itime) values (1, " . time () . ")", $link );			
		};
		Header ( "Location: index.php?type=acl" );
		exit ();
